...
class MainTest {
    ...
    @BeforeEach
    public void setUp() {
        deck = new Deck(123);  // use random seed for shuffling
    ...
    public void mainTest() {
        ...
        assertTrue(true);
        Main.playerTurn(player, dealer, new Scanner("0"));
        Main.dealerTurn(player, dealer);
        assertTrue(true);
        Main.round(1, player, dealer, new Scanner("0000000000000000000000000000"));
        assertEquals(1, player.win);
    }
    ...
    public void testHandToString() {
        player.hand.takeCard(deck.drawCard());
        assertEquals("Девятка Трефы (9)", player.hand.toString());
    }
}